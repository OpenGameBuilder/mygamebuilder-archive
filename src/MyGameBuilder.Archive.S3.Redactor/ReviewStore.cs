...
public sealed class ReviewStore
{
    ...
    public ReviewStateDto GetStateDto(RedactorOptions options)
    {
        using var connection = Open();
        var counts = GetCounts(connection);
        var currentIndex = Math.Clamp(GetCurrentIndex(connection), 0, Math.Max(0, counts.Total - 1));
        if (counts.Total == 0)
        {
            currentIndex = 0;
        }

        SetCurrentIndex(connection, currentIndex);
        var current = counts.Total == 0 ? null : GetCandidateAt(connection, currentIndex);
        return new ReviewStateDto(
            ...
            currentIndex,
            counts,
            current is null ? null : ToDto(current, currentIndex));
    }
    ...
    private SqliteConnection Open()
    {
        ...
        connection.Open();
        ArchiveDb.Execute(connection, "PRAGMA foreign_keys = ON; PRAGMA busy_timeout = 5000; PRAGMA journal_mode = WAL;");
        return connection;
    }
