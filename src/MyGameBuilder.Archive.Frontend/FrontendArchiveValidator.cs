...
public static class FrontendArchiveValidator
{
    ...
    [
        ...
        "frontend_discovered_url",
        "frontend_discovered_url_source"
    ];
    ...
    private static void ValidateCounts(SqliteConnection connection, ICollection<string> errors, ICollection<string> warnings)
    {
        ...
        var replayErrors = Count(connection, "SELECT COUNT(*) FROM frontend_capture WHERE replay_error IS NOT NULL;");
        if (replayErrors != 0)
        {
            warnings.Add($"{replayErrors} captures have recorded replay errors and no archived body.");
        }
