...
public sealed class SqliteFrontendArchiveStore : IDisposable
{
    ...
    public void Initialize()
    {
        ...
        UpsertArchiveInfo("schema_version", "1");
        UpsertArchiveInfo("content_scope", "Wayback CDX captures, raw replay headers and bodies, and URL-like references discovered in replay bodies");
    }
    ...
    public void ResetCapture()
    {
        Sqlite.ExecuteNonQuery(
            _connection,
            """
            DELETE FROM frontend_discovered_url_source;
            DELETE FROM frontend_discovered_url;
    ...
    public IReadOnlyList<PendingReplayCapture> GetPendingReplays(int take)
    {
        using var command = _connection.CreateCommand();
        command.CommandText =
            """
    ...
    public void InsertReplay(long captureId, ReplayDownload download, IReadOnlyList<UrlMatch> discoveredUrls)
    {
        using var transaction = _connection.BeginTransaction();
        ...
        InsertDiscoveredUrls(captureId, contentId, discoveredUrls, transaction);
        transaction.Commit();
    }
    ...
    {
        using var transaction = _connection.BeginTransaction();
        UpsertArchiveInfo("capture_tool", "MyGameBuilder.Archive.Frontend", transaction);
        ...
        UpsertArchiveInfo("discovered_url_count", Count("frontend_discovered_url").ToString(System.Globalization.CultureInfo.InvariantCulture), transaction);
        transaction.Commit();
    }
    ...
    private void ValidateExistingExcludes(IReadOnlyList<FrontendExclude> excludes)
    {
        ...
        command.CommandText =
            """
            SELECT line_number, exclude_value, exclude_text, canonical_prefix, host_path_prefix
    ...
    private void InsertExcludes(IReadOnlyList<FrontendExclude> excludes, SqliteTransaction transaction)
    {
        ...
        command.CommandText =
            """
            INSERT INTO frontend_exclude(line_number, exclude_value, exclude_text, canonical_prefix, host_path_prefix)
            VALUES ($line_number, $exclude_value, $exclude_text, $canonical_prefix, $host_path_prefix);
            """;
        AddParameter(command, "$line_number");
    ...
    private long GetOrInsertCapture(long resourceId, CdxCapture capture, SqliteTransaction transaction)
    {
        using (var insert = _connection.CreateCommand())
        {
            ...
            insert.CommandText =
                """
                INSERT OR IGNORE INTO frontend_capture(
    ...
    private void InsertDiscoveredUrls(long captureId, long contentId, IReadOnlyList<UrlMatch> urls, SqliteTransaction transaction)
    {
        var grouped = new Dictionary<string, (UrlMatch Match, string ResolvedKey, int Count)>(StringComparer.Ordinal);
        foreach (var match in urls)
        {
            var resolvedKey = match.ResolvedCanonicalUrl ?? match.ResolvedUrl ?? string.Empty;
            var key = match.RawText + "\u001f" + resolvedKey;
            grouped.TryGetValue(key, out var existing);
            grouped[key] = existing.Count == 0
                ? (match, resolvedKey, 1)
                : (existing.Match, existing.ResolvedKey, existing.Count + 1);
        }

        foreach (var group in grouped.Values)
        {
            var discoveredId = GetOrInsertDiscoveredUrl(captureId, contentId, group.Match, group.ResolvedKey, group.Count, transaction);
            using var source = _connection.CreateCommand();
            source.Transaction = transaction;
            source.CommandText =
                """
                INSERT INTO frontend_discovered_url_source(discovered_url_id, capture_id, content_id, occurrence_count)
                VALUES ($discovered_url_id, $capture_id, $content_id, $occurrence_count)
                ON CONFLICT(discovered_url_id, capture_id) DO UPDATE SET
                    occurrence_count = occurrence_count + excluded.occurrence_count;
                """;
            source.Parameters.AddWithValue("$discovered_url_id", discoveredId);
            source.Parameters.AddWithValue("$capture_id", captureId);
            source.Parameters.AddWithValue("$content_id", contentId);
            source.Parameters.AddWithValue("$occurrence_count", group.Count);
            source.ExecuteNonQuery();
        }
    }

    private long GetOrInsertDiscoveredUrl(
        long captureId,
        long contentId,
        UrlMatch match,
        string resolvedKey,
        ...
        SqliteTransaction transaction)
    {
        using (var insert = _connection.CreateCommand())
        {
            ...
            insert.CommandText =
                """
                INSERT INTO frontend_discovered_url(
                    raw_text, resolved_url, resolved_canonical_url, resolved_url_key,
                    first_capture_id, first_content_id, occurrence_count, first_seen_utc
                )
                VALUES (
                    $raw_text, $resolved_url, $resolved_canonical_url, $resolved_url_key,
                    $first_capture_id, $first_content_id, $occurrence_count, $first_seen_utc
                )
                ON CONFLICT(raw_text, resolved_url_key) DO UPDATE SET
                    occurrence_count = occurrence_count + excluded.occurrence_count;
                """;
            insert.Parameters.AddWithValue("$raw_text", match.RawText);
            insert.Parameters.AddWithValue("$resolved_url", (object?)match.ResolvedUrl ?? DBNull.Value);
            insert.Parameters.AddWithValue("$resolved_canonical_url", (object?)match.ResolvedCanonicalUrl ?? DBNull.Value);
            insert.Parameters.AddWithValue("$resolved_url_key", resolvedKey);
            insert.Parameters.AddWithValue("$first_capture_id", captureId);
            insert.Parameters.AddWithValue("$first_content_id", contentId);
            insert.Parameters.AddWithValue("$occurrence_count", occurrenceCount);
            insert.Parameters.AddWithValue("$first_seen_utc", FormatUtc(DateTimeOffset.UtcNow));
            insert.ExecuteNonQuery();
        }
        ...
        select.CommandText =
            """
            SELECT discovered_url_id
            FROM frontend_discovered_url
            WHERE raw_text = $raw_text
              AND resolved_url_key = $resolved_url_key;
            """;
        select.Parameters.AddWithValue("$raw_text", match.RawText);
        select.Parameters.AddWithValue("$resolved_url_key", resolvedKey);
        return Convert.ToInt64(select.ExecuteScalar() ?? throw new ArchiveFatalException($"Could not insert discovered URL '{match.RawText}'."));
    }
    ...
    private const string SchemaSql =
        ...
        CREATE TABLE IF NOT EXISTS frontend_exclude (
            ...
            UNIQUE(line_number, exclude_value)
        ) STRICT;
        ...
        CREATE TABLE IF NOT EXISTS frontend_capture (
            ...
            cdx_length INTEGER NULL CHECK (cdx_length IS NULL OR cdx_length >= 0),
        ...
        CREATE TABLE IF NOT EXISTS frontend_discovered_url (
            discovered_url_id INTEGER PRIMARY KEY,
            raw_text TEXT NOT NULL COLLATE BINARY,
            resolved_url TEXT NULL COLLATE BINARY,
            resolved_canonical_url TEXT NULL COLLATE BINARY,
            resolved_url_key TEXT NOT NULL COLLATE BINARY,
            ...
            first_seen_utc TEXT NOT NULL COLLATE BINARY,
            UNIQUE(raw_text, resolved_url_key)
        ) STRICT;

        CREATE TABLE IF NOT EXISTS frontend_discovered_url_source (
            discovered_url_id INTEGER NOT NULL
                REFERENCES frontend_discovered_url(discovered_url_id)
                ON DELETE RESTRICT,
            capture_id INTEGER NOT NULL
                REFERENCES frontend_capture(capture_id)
                ON DELETE RESTRICT,
            content_id INTEGER NOT NULL
                ...
                ON DELETE RESTRICT,
            occurrence_count INTEGER NOT NULL CHECK (occurrence_count > 0),
            PRIMARY KEY(discovered_url_id, capture_id)
        ) STRICT, WITHOUT ROWID;
        ...
        CREATE INDEX IF NOT EXISTS ix_frontend_discovered_resolved
            ON frontend_discovered_url(resolved_canonical_url)
            WHERE resolved_canonical_url IS NOT NULL;
