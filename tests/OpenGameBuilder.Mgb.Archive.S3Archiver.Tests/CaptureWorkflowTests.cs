...
public sealed class CaptureWorkflowTests
{
    ...
    [Fact]
    public async Task CaptureWorkflowResumeTreatsExistingValidOutputAsComplete()
    {
        var directory = Path.Combine(Path.GetTempPath(), "mgb-archive-tests", Guid.NewGuid().ToString("N"));
        var output = Path.Combine(directory, "archive.sqlite");
        var work = Path.Combine(directory, "work");
        ...
        await RunWorkflowAsync(output, work, resume: false, new FakeS3Handler());

        await RunWorkflowAsync(output, work, resume: true, new ThrowingHandler());

        Assert.True(File.Exists(output));
    }

    [Fact]
    public async Task CaptureWorkflowResumePublishesFinalizedInProgressArchive()
    {
        var directory = Path.Combine(Path.GetTempPath(), "mgb-archive-tests", Guid.NewGuid().ToString("N"));
        var output = Path.Combine(directory, "archive.sqlite");
        var inProgress = output + ".inprogress.sqlite";
        var work = Path.Combine(directory, "work");
        Directory.CreateDirectory(directory);

        await RunWorkflowAsync(output, work, resume: false, new FakeS3Handler());
        File.SetAttributes(output, File.GetAttributes(output) & ~FileAttributes.ReadOnly);
        File.Move(output, inProgress);

        await RunWorkflowAsync(output, work, resume: true, new ThrowingHandler());

        Assert.True(File.Exists(output));
        Assert.False(File.Exists(inProgress));
    }

    private static async Task RunWorkflowAsync(string output, string work, bool resume, HttpMessageHandler handler)
    {
        await using var diagnostics = DiagnosticsWriter.Create(work);
        using var loggerFactory = LoggerFactory.Create(static _ => { });
        ...
        var s3Client = new S3ArchiveClient(httpClient, new Uri("https://example.test"), "bucket", loggerFactory.CreateLogger<S3ArchiveClient>());
        var workflow = new CaptureWorkflow(
            new ArchiveOptions("bucket", new Uri("https://example.test"), output, work, Concurrency: 2, Resume: resume, Replace: false),
            s3Client,
            diagnostics,
            loggerFactory.CreateLogger<CaptureWorkflow>());
    ...
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken) =>
            throw new InvalidOperationException("HTTP should not be used for this resume path.");
    }
}
